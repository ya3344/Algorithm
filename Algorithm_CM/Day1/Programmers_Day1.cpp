...
int solution6(int n) {
	...
	for (int i = 3; i <= n; i += 2)
	{
		...
		int size = (int)sqrt(vecPrime.size());
...
{
	// 문제 1. 2016년
	cout << solution1(11, 9) << endl;

	// 문제 2. 같은 숫자는 싫어
	vector<int> arr = solution2(vector<int>({ 1, 1, 3, 3, 0, 1, 1 }));
	for(int out : arr)
		cout << out << endl;

	// 문제 3. 두 정수 사이의 합
	cout << solution3(3, 3) << endl;

	// 문제 4. 문자열 내 p와 y의 개수
	cout << solution4("pPoooyY") << endl;

	// 문제 5. 문자열 다루기 기본
	cout << solution5("123456") << endl; 

	// 문제 6. 소수 찾기
	cout << solution6(30) << endl;
